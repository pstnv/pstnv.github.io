...
function checkWidth () {
    ...
    // 1 случай        
        ...
        showCards.addEventListener('click', () => {
            ...
            hideCardsBtn.querySelector('.figureArrow').classList.add('scalefigureArrow');
        });
        ...
        hideCardsBtn.addEventListener('click', () => {
            ...
            hideCardsBtn.querySelector('.figureArrow').classList.remove('scalefigureArrow');
            ...
            window.scrollTo({ top: elementOffset - elementMarginTop }) || document.documentElement.scrollTo({ top: elementOffset - elementMarginTop
            });
        });
    }
    // 2 случай
...
cards.forEach(card => {
	card.querySelector('.moreInfo').addEventListener('click', () => {
		...
		setTimeout(() => {
			...
			card.querySelector('.headerFront').classList.add('headerFrontHidden');
		}, 0);
		setTimeout(() => {
...
function showFrontCard() {
	cards.forEach(card => {
		card.querySelector('.headerFront').classList.remove('headerFrontHidden');
...
window.addEventListener('click', event => {
	if (!event.target.classList.contains('moreInfo') & !event.target.classList.contains('linkVisit')) {
		showFrontCard();
	}
});
...
linkHome.addEventListener('click', () => {
	...
	window.scrollTo({ top: 0 }) || document.documentElement.scrollTo({ top: 0 });
});